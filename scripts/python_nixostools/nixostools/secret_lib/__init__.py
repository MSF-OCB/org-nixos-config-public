...
def encrypt_symmetric_string(key: bytes, string_to_encrypt: str) -> str:
    return encrypt_symmetric(key, string_to_encrypt.encode(UTF8))


def encrypt_symmetric(key: bytes, bytes_to_encrypt: bytes) -> str:
    box = SecretBox(key)
...
def decrypt_symmetric(key: bytes, encrypted_secrets: str) -> str:
    box = SecretBox(key)
    return decrypt(box, encrypted_secrets).decode(UTF8)


# takes a string of bytes and returns an encrypted version.
def encrypt_asymmetric(pubkey: PublicKey, bytes_to_encrypt: bytes) -> str:
...
def decrypt_asymmetric(privkey: PrivateKey, encrypted_key: str) -> bytes:
    box = SealedBox(privkey)
    return decrypt(box, encrypted_key)


def decrypt(box: Any, ciphertext: str) -> bytes:
    return box.decrypt(ciphertext, encoder=Base64Encoder)
...
def extract_curve_public_key(openssh_public_key: str) -> PublicKey:
    ...
    pub_bytes = bytes_after(
        OPENSSH_PUBLIC_KEY_SIGNATURE, PUBLIC_KEY_LENGTH, openssh_pub_bytes
    )
...
def extract_curve_private_key(priv_key: str) -> PrivateKey:
    ...
    priv_bytes = bytes_after(
        OPENSSH_PRIVATE_KEY_SIGNATURE, PRIVATE_KEY_LENGTH, openssh_priv_bytes
    )
...
def bytes_after(signature: bytes, length: int, bytestr: bytes) -> bytes:
