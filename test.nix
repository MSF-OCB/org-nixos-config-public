...
in
{
  demo-host = runNixOSTest flakeInputs
    (
      { pkgs, ... }:
      {
        name = "demo-host";
        nodes = {
          machine = {
            imports = [
              ./demo-org-config/hosts/demo-host.nix
        ...
        testScript = ''
          demo-host.start()
          demo-host.wait_for_unit("multi-user.target")
        '';
      }
    )
    { inherit pkgs; };
    ...
    pythonTest.makeTest
      {
        ...
        testScript =
          ''
            ...
            assert "TEST" in machine.succeed("cat /var/lib/msf-ocb/id_tunnel")
            machine.shutdown()
          '';
      };
}
