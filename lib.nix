  ...
  libOverlay = final: prev:
    ...
    let
      ...
      mergeDisjoint =
        let
          merge = name: values:
            ...
            then throw "mergeDisjoint: got multiple values for attribute ${name}"
      ...
      */
      ...
      toHostPath = hostname: ./demo-org-config/hosts/${hostname} + ".nix";
      ...
      evalHost = defaultModules: flakeInputs:
        # The arguments below can be overridden by the hostOverrides argument
        { hostname, nixpkgs, extraModules ? [ ], extraSpecialArgs ? { }, ... }:
        let
          traceInfo = compose [
      ...
      mkNixosConfigurations =
        ...
        let
          ...
          evalHosts = lib.mapAttrs (hostname: args:
            evalHost defaultModules flakeInputs ({ inherit hostname; } // args)
          );
          ...
          hostDefinitions = lib.flip lib.mapAttrs hosts (hostname: config:
            let
            ...
            in
            lib.flip lib.zipAttrsWith [ config hostOverride ] (name: values: {
      ...
      allHosts = compose [
        (lib.mapAttrs' (name: _: let host = lib.removeSuffix ".nix" name; in lib.nameValuePair host host))
        (lib.filterAttrs (name: type: type == "regular" && lib.hasSuffix ".nix" name))
        builtins.readDir
      ] ./demo-org-config/hosts;
      ...
      runNixOSTest = flakeInputs: test: { pkgs }:
        let
          inherit (pkgs) lib;
        ...
        (nixos-lib.runTest {
          ...
          node = {
            inherit pkgs;
            specialArgs = {
              flakeInputs = flakeInputs // { nixpkgs = pkgs; };
              inherit (pkgs) lib;
            };
          };
          imports = [ test ];
      ...
      exported = {
        inherit compose applyTwice filterEnabled mergeDisjoint
          concatMapAttrsToList stringNotEmpty mapListToAttrs
          findDuplicates findDuplicateMappings indentStr
          traceImportJSON ifPathExists keyIfExists
          nixosVersionOf toHostPath recursiveMerge
          allHosts mkNixosConfigurations
          getSlice runNixOSTest;
      };
    ...
    in
    # We can only use builtins here, otherwise we run into infinite recursion.
    if builtins.length duplicateAttributes > 0
    then throw "newly added attribute shadows an existing one: ${builtins.concatStringsSep ", " duplicateAttributes}"
    else exported // { inherit types; };
in
libOverlay
